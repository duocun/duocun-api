...
export class AccountController extends Controller {
    ...
    async list(req: Request, res: Response) {
        ...
        if (req.headers && req.headers.filter && typeof req.headers.filter === 'string') {
            where = (req.headers && req.headers.filter) ? JSON.parse(req.headers.filter) : null;
        }
        ...
        where = this.accountModel.convertIdFields(where);

        if (params && params.keyword) {
            where = { ...where, username: { '$regex': params.keyword } };
        }

        const accounts = await this.accountModel.find(where);
