import { PaymentError, ResponseStatus, PaymentAction } from "../client-payment";
...
class StripePayment extends Payment {
    ...
    async payByStripe(paymentActionCode: string, paymentMethodId: string, accountId: string, accountName: string,
        ...
        const orders = await this.orderModel.find({ paymentId, status: { $nin: [OrderStatus.BAD, OrderStatus.DELETED] }, paymentStatus: { $ne: PaymentStatus.PAID } });
